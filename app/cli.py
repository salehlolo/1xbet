...
def command_report_performance(args: argparse.Namespace) -> None:
    settings = validate_settings(get_settings())
    db = Database(settings.db_path)
    rows = db.report_performance(days=args.days)
    if not rows:
        print("No CLV data found for selected period.")
        return
    print("market,count,avg_clv")
    for market, count, avg_clv in rows:
        print(f"{market},{count},{avg_clv:.4f}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    report_parser = subparsers.add_parser("report-performance", help="Report CLV performance")
    report_parser.add_argument("--days", type=int, default=7)
    report_parser.set_defaults(func=command_report_performance)
